...
<%@page import="entity.Orders,entity.OrderDetail,entity.Book, java.util.Enumeration, jakarta.servlet.http.HttpSession, java.util.Vector"%>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        ...
        <div class="justify-content-center">
            <form action="OrderController?service=listAll" method="post">
                <p class="text-center align-items-center mt-3 mb-3" style="caption-side: top; font-weight: 900; font-size: 130%">All Orders</p>
                <%
                   Vector<Orders> vector=
                    (Vector<Orders>)request.getAttribute("orders");
                   Vector<OrderDetail> vectorDetail = 
                           (Vector<OrderDetail>)request.getAttribute("orderDetail");
                   Vector<Book> vectorBook = 
                           (Vector<Book>)request.getAttribute("book");
                   for (int i = 0;i<vector.size();i++) {
                    OrderDetail thisOrder = new OrderDetail();
                    Book thisBook = new Book();
                        for(OrderDetail od : vectorDetail){
                ...
                <div style="border-width:3px;border-style: solid">
                    ...
                    <div style="background-color: lightyellow;color: red;text-align: right;border-width:1px;border-color: gray;border-style: solid">
                        <div>
                            Trả tiền: <%=thisOrder.getPrice()*thisOrder.getQuantity()%> 
                        </div>           
                    </div>
                </div>
